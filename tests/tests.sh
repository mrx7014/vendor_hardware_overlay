...
find . -name AndroidManifest.xml | while read -r manifest; do
    ...
    # Everything after that is specifically for static overlays, targeting framework-res
    ...
    # Ensure power profile only contains expected types
    ...
    # Ensure user didn't leave some public.xml
    find "$folder" -name \*.xml | while read -r xml; do
